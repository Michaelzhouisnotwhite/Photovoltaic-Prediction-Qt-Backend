...
class Exp_Main(Exp_Basic):
    ...
    def _build_model(self):
        ...
        if self.args.use_multi_gpu and self.args.use_gpu:
            ...
            print("the model devices:{}\n".format(model.device_ids))
        return model
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if (i + 1) % 100 == 0:
                    logger.info("\titers: {0}, epoch: {1} | loss: {2:.7f}".format(
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if i % 20 == 0:
                    ...
                    B, L, D = input.shape
                    input = test_data.inverse_transform(input.reshape(-1, D)).reshape(B, -1, D)
                    mean, std = test_data.scaler.mean_, test_data.scaler.with_std
                    true = true * std + mean
                    pred = pred * std + mean

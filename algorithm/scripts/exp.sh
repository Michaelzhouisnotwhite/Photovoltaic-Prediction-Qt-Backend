...
# python run.py --root_path {} --data_path {} (--model {"Informer" "FEDformer" "Autoformer"}) --seq_len {96 偶数} 
#       --data {名字标记} --pred_len {偶数}  
run_PV(){
    root_path="../dataset/PV/"
    data_path="$1"
    model="$2" #NOTE
    data="$3" # nothing
    exp_notes="$4" #NOTE # nothing default ""
    seq_len="$5" #NOTE # 自定义 偶数 96
    label_len=$(($seq_len/2)) # 
    pred_len="$6" # 输出长度 even 
    factor="$7" # 3
    enc_in=13
    dec_in=13
    c_out=1
